...
func isValidArrangement(arrangement []string, towels []string, bufLen int) bool {
	...
	for len(buffer) < bufLen {
		buffer = buffer + arrangement[char]
...
func Part1() (time.Duration, time.Duration, int) {
	ParseStart := time.Now()
	raw := fileparse.FileParse("day19/TestInput.txt")
	towelRaw := fileparse.FileParse("day19/TestTowels.txt")
	ParseTime := time.Since(ParseStart)
	P1Start := time.Now()

	//Insert additional processsing here
	input := make([][]string, len(raw))
	for _, i := range raw {
		input = append(input, strings.Split(i, ""))
	}
	tmp := ""
	for _, i := range towelRaw {
		tmp = tmp + i
	}
	towels = strings.Split(tmp, ", ")
